<div class="form-group">
    ...
    @for($i = 0; $i <= 3; $i++)
        <input type="text" name="options[{{ $i }}][name]" class="form-control {{ $errors->has('options'. $i . '.name') ? 'is-invalid' : '' }}"
               id="options.{{ $i }}.name" placeholder="Enter Answer" value="{{ old('options.'. $i . '.name') }}">
        @if ($errors->has('options.'. $i . '.name'))
            <span class="invalid-feedback">{{ $errors->first('options.'. $i . '.name') }}</span>
        @endif
        <div class="form-check">
            <input type="radio" name="options[{{ $i }}][is_answer]" value="1" tt="{{old('options.'. $i . '.is_answer')}}" {{ old('options.'. $i . '.is_answer') == 1 ? 'checked' : '' }} class="form-check-input" id="options.{{ $i }}.is_answer">
